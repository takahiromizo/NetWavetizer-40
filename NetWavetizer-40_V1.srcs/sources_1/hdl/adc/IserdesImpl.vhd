...
entity IserdesImpl is
  generic
  (
    kSysW             : integer := 1;          -- width of the data for the system
    kDevW             : integer := 10;         -- width of the data for the device
...
begin
  u_Rx_IBUFDS_inst : IBUFDS
    ...
    port map (
      ...
      IB  => dInFromPinN             -- Diff_p buffer input (connect directly to top-level port)
      );
  ...
  u_ISERDESE2_master : ISERDESE2
    ...
    -- Input Data: 1-bit (each) input: ISERDESE2 data input ports
      ...
      DDLY                   => data_in_from_pin_delay, -- 1-bit input: High speed negative edge output clock
