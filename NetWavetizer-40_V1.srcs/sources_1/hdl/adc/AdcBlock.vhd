...
entity AdcBlock is
  ...
  port(
    ...
    -- data output--
    AdcData                    : out std_logic_vector(kNumAdcBit*kNumAdcInputBlock-1 downto 0);
...
architecture RTL of AdcBlock is
  ...
  -- Ring Buffer ------------------------------------------------------------
  signal rb_in                           : std_logic_vector(kNumAdcBit*kNumAdcInputBlock-1 downto 0); -- 32*12
  ...
  signal reg_rv_ringbuf                  : std_logic;
  signal rv_ringbuf                      : std_logic;
  
  signal read_ptr                        : std_logic_vector(kWidthCoarseCount-1 downto 0);
  signal rb_out                          : std_logic_vector(kNumAdcBit*kNumAdcInputBlock-1 downto 0); -- 32*12
  
  COMPONENT ringbuffer_adc
    PORT (
      clka  : IN  STD_LOGIC;
      wea   : IN  STD_LOGIC_VECTOR(0 DOWNTO 0);                              -- write enable, always "1"
      addra : IN  STD_LOGIC_VECTOR(kWidthCoarseCount-1 DOWNTO 0);
      dina  : IN  STD_LOGIC_VECTOR(kNumAdcBit*kNumAdcInputBlock-1 DOWNTO 0); -- 32*12
      clkb  : IN  STD_LOGIC;
      rstb  : IN  STD_LOGIC;
      enb   : IN  STD_LOGIC;
      addrb : IN  STD_LOGIC_VECTOR(kWidthCoarseCount-1 DOWNTO 0);
      doutb : OUT STD_LOGIC_VECTOR(kNumAdcBit*kNumAdcInputBlock-1 DOWNTO 0)  -- 32*12
      );
  END COMPONENT;
  ...
  signal reg_coarse_counter              : std_logic_vector(kWidthCoarseCount-1 downto 0);
  ...
  signal adc_ch                          : std_logic_vector(kWidthAdcChannel-1 downto 0);
  ...
  -- Event Summary FIFO -----------------------------------------------------
  signal din_evsum                       : std_logic_vector(kWidthEvtSummary-1 downto 0);
  signal dout_evsum                      : std_logic_vector(kWidthEvtSummary-1 downto 0);
  ...
  --attribute mark_debug of read_valid          : signal is "true";
  -- Event summary --
  ...
  --attribute mark_debug of din_evsum           : signal is "true";
  --attribute mark_debug of dout_evsum          : signal is "true";
...
begin
  ...
  -- body
  -- ========================================================================
  readyAdcRO         <= adcro_is_ready(0) and adcro_is_ready(1) and adcro_is_ready(2) and adcro_is_ready(3); 
  offset_ch          <= std_logic_vector(to_unsigned(initCh, 5));
  ...
  u_buf_local : process(rst, clkRegion)
  begin
    if(rst = '1') then
      cstop_issued   <= '0';
    elsif(clkRegion'event AND clkRegion = '1') then
      cstop_issued   <= cStop;
    end if;
  end process;
  ...
  begin
    if(rst = '1') then
      ...
      reg_rv_ringbuf         <= rv_ringbuf;
      reg_data_bit           <= data_bit;
      reg_coarse_counter     <= coarse_counter;
      for i in 0 to kNumAdcInputBlock-1 loop
        bufd_ring2chfifo(i)  <= reg_data_bit & reg_coarse_counter & rb_out(kNumAdcBit*(i+1)-1 downto kNumAdcBit*i);
        --bufd_ring2chfifo(i)  <= data_bit & coarse_counter & rb_out(kNumAdcBit*(i+1)-1 downto kNumAdcBit*i);
        bufwe_ring2chfifo(i) <= (reg_rv_ringbuf AND (NOT pgfull_fifo(i))) OR we_endevent;
  ...
  -- Write data from Ring Buffer to channel buffer --------------------------
  u_write_ptr : process(rst, clkRegion)
  begin
    if(rst = '1') then
      write_ptr    <= (others => '0');
    elsif(clkRegion'event AND clkRegion = '1') then
  ...
  begin
    if(rst = '1') then
      ...
      din_block_buffer_buf  <= magicWord & adc_ch & dout_chfifo(id)(kIndexAdcDataBit-1 downto 0);
